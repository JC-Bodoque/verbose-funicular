...
public class EcoRMinimizer {
    ...
    public static int addStateMin(int p, int nn, int[] num, IDFA a, IDFA b) {
        num[p] = nn;
        b.terminal.toArray()[nn] = a.terminal.toArray()[p];
    ...
    public static int renumberStates(int[] e, int[][] sil, int nn,
        ...
        for (int k = 1; k < e.length; k++) {
            q = e[k];
            if (!equalSignature(sil[k], sil[k - 1])) {
     ...
     */
    public static void computeSignatures(int[] som, int[][] sig, int[] num, IntQueue shl, IDFA a) {
        int i = 0;
...
//                c[0] = 1;
     ...
     */
    public static boolean equalSignature(int[] a, int[] b) {
     ...
     * is the queue of states at height <code>r</code>.
     *
     * @param h the array of heights
     * @return the array of lists of states at given height.
     **/
    ...
    public static IDFA minimize(IDFA a) {
        ...
        for (int r = 1; r < pi.length; r++) {
            ...
            for (IntList ld = lambda.dans.front; ld != null; ld = ld.next) {
                ...
                if (states.size() == 1) {
                    // a new state of the minimal automaton
