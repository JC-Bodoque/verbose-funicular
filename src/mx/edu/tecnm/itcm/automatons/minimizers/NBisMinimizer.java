 ...
 */
public class NBisMinimizer implements Minimizer {
    ...
    public Partition partition(IDFA a, Partition p) {
        Partition q = refine(a, p);
        while (p.index != q.index) {
            if (verbose) System.out.println("while loop");
            p = q;
            q = refine(a, q);
        }
        return q;
    ...
    public int[] partition(IDFA a, int[] p) {
        int[] q = refine(a, p);
        while (index(p) != index(q)) {//System.out.println("index = "+ index(q));
            p = q;
            q = refine(a, q);
        }
        return q;
    ...
    public int[] refine(IDFA a, int[] c) { // refines the partition c
        int[] r = new int[a.nbStates];
        int[] d = new int[a.nbStates];
        int m = 0;
        for (int q = 0; q < a.nbStates; q++) {
            if (verbose)
                if (q % 10000 == 0) System.out.println("q = " + q);
            boolean found = false;
            for (int p = 0; p < m; p++)
                if (equiv(a, r[p], q, c)) {
                    d[q] = d[r[p]];
